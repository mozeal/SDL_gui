...
checked(false)
{
    ...
    int cSize = ((int)(textView->rectView.height()/GUI_scale)) & ~1;
...
void GUI_CheckBox::drawInteract() {
    if( showInteract && getInteract() ) {
        GUI_Rect *rect = GUI_MakeRect(checkView->topLeft.x, checkView->topLeft.y, checkView->rectView.width(), checkView->rectView.height());
        
        GUI_FillRoundRect(rect->x, rect->y, rect->width(), rect->height(), checkView->getCorner(), cHightLightInteract);
    }
}
