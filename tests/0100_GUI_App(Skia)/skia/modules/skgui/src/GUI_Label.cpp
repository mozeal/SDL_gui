...
void GUI_Label::setTextColor( SkColor c ) {
    _textColor = c;
    if( textView ) {
...
void GUI_Label::setLayout( int l ) {
    ...
    if( _layout == GUI_LAYOUT_HORIZONTAL ) {
        ...
        if( iconView && textView ) {
            if( iconView->rectView.height() > textView->rectView.height() ) {
                txm = (iconView->rectView.height() - textView->rectView.height()) / (2 * GUI_scale);
            }
            else {
                icm = (textView->rectView.height() - iconView->rectView.height()) / (2 * GUI_scale);
            }
        }
