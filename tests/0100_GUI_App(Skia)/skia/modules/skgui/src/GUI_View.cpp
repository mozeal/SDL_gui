...
bool GUI_View::eventHandler(SDL_Event*event) {
    ...
    switch( event->type ) {
        case GUI_UpdateSize:
            ...
            topLeft.set(ox * GUI_scale, oy * GUI_scale);
...
//                GUI_Rect rClip, rView;
//                GUI_GetViewport(rView);
//                GUI_GetClipRect(rClip);
...
void GUI_View::predraw() {
    ...
    if( isFocus() && focusBorder > 0 ) {
        ...
        if (parent) {
            ...
            rectClip = GUI_Rect(0, 0, rectView.w+(f*2), rectView.h+(f*2)).getIntersection(parent_clip);
        ...
        GUI_SetViewport(GUI_Rect(rectView.x-f, rectView.y-f, rectView.w+(f*2), rectView.h+(f*2)));
    ...
    if (parent) {
        ...
        rectClip = GUI_Rect(0, 0, rectView.w, rectView.h).getIntersection(parent_clip);
    ...
    GUI_SetViewport(GUI_Rect(rectView.x, rectView.y, rectView.w, rectView.h));
    GUI_SetClipRect(rectClip);
    clear();
}
...
//        rectClip = r.getIntersection(rectClip);
//        GUI_SetViewport(GUI_Rect(rectView.x, rectView.y, rectView.w, rectView.h));
//        GUI_SetClipRect(rectClip);
//
//
...
////        SkRect clip(r.toSkRect());
////        if(clip.intersect(rectClip.toSkRect())){
////            rectClip.set(clip.x(), clip.y(), clip.width(), clip.height());
////        }
////        else{
////            rectClip.set(0,0,0,0);
////        }
//
////        rectClip = r.getIntersection(rectClip);
//
...
//////                                                         rectClip.h));
////        SkCanvas * canvas = GUI_App::getInstance()->getCanvas();
////        canvas->clipRect(rectClip.toSkRect(), true);
////#endif
//
////        rectClip = r.getIntersection(rectClip);
////        GUI_SetViewport(GUI_Rect(rectView.x, rectView.y, rectView.w, rectView.h));
...
void GUI_View::clear(GUI_Rect *rect) {
    ...
    {
        if(SkColorGetA(_backgroundColor) == 0)
            return;
        if(!corner)
...
void GUI_View::drawFocus() {
    ...
    if ( SkColorGetA(_backgroundColor) != 0) {
        SkColor color = _backgroundColor;
//        color.a = 96;
//        color.r &= 0x7F;
//        color.g &= 0x7F;
//        color.b &= 0x7F;
        color = SkColorSetARGB(96, SkColorGetR(color)&0x7f, SkColorGetG(color)&0x7f, SkColorGetB(color)&0x7f);
        int corner = getCorner();
        if (corner != 0) {
...
//    if (SDL_PointInRect(&pt, &rectView)) {
    if(rectView.toSkRect().contains(pt.x, pt.y)){
        // GUI_Log( "Hit in %s\n", title.c_str() );
        if (bRecursive) {
            if( children.size() > 0 ) {
