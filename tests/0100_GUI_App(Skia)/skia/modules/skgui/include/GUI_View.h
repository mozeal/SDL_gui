...
protected:
    ...
    SkColor _backgroundGradientColor1;
    SkColor _backgroundGradientColor2;
...
public:
    ...
    int getWidth() { return rectView.width() / GUI_scale; }
    int getHeight() { return rectView.height() / GUI_scale; }

    eBackgroundMode backgroundMode() { return mBackgroundMode; }

    virtual void setBackgroundColor( SkColor c ) { _backgroundColor = c; mBackgroundMode = eSolidColor; }
    virtual SkColor getBackgroundColor() { return _backgroundColor; }

    void setBackgroundGradient(SkColor aColor1, SkColor aColor2, int aOrientation = GUI_LAYOUT_VERTICAL)
    ...
    std::pair<SkColor, SkColor> getBackgoundColors() { return std::pair<SkColor, SkColor>(_backgroundGradientColor1, _backgroundGradientColor2); }

    void setBackgroundTexture(const std::string& aFileName) {
//        _backgroundTexture = IMG_LoadTexture(GUI_renderer, aFileName.c_str());
//        mBackgroundMode = eTexture;
    }

    virtual void setTextColor( SkColor c ) { _textColor = c; }
    virtual SkColor getTextColor() { return _textColor; }

    virtual void setBorderColor( SkColor c ) { _borderColor = c; }
    virtual SkColor getBorderColor() { return _borderColor; }
