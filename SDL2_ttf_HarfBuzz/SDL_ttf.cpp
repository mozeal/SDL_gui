...
static hb_direction_t g_hb_direction = hb_direction_t::HB_DIRECTION_LTR;
static hb_script_t    g_hb_script = hb_script_t::HB_SCRIPT_THAI;  //HB_SCRIPT_UNKNOWN;
...
static FT_Error Load_Glyph( TTF_Font* font, Uint16 idx, c_glyph* cached, int want )
{
    FT_Face face;
    FT_Error error;
    FT_GlyphSlot glyph;
    ...
    cached->cached_idx = idx;
    cached->is_cached = SDL_TRUE;

    return 0;
}

static FT_Error Find_Glyph( TTF_Font* font, Uint16 idx, int want )
{
    int retval = 0;
    ...
    if (font->current->is_cached && font->current->cached_idx != idx)
...
int TTF_SizeUTF8(TTF_Font *font, const char *text, int *w, int *h)
{
    ...
    {
        const int idx = hb_glyph_info[i].codepoint;
        error = Find_Glyph(font, idx, CACHED_METRICS);
        if ( error ) {
            TTF_SetFTError("Couldn't find glyph", error);
                ...
                const char *text, SDL_Color fg)
{
    ...
    {
        const int idx = hb_glyph_info[i].codepoint;
        error = Find_Glyph(font, idx, CACHED_METRICS|CACHED_BITMAP);
        if ( error ) {
            TTF_SetFTError("Couldn't find glyph", error);
            SDL_FreeSurface( textbuf );
            hb_buffer_destroy(hb_buffer);
        ...
        hb_position_t gpos_xoffset = hb_glyph_position[i].x_offset >> 6;
        hb_position_t gpos_yoffset = hb_glyph_position[i].y_offset >> 6;
        ...
        for ( row = 0; row < current->rows; ++row ) {
            ...
            dst = (Uint8*) textbuf->pixels +
            (row+glyph->yoffset-gpos_yoffset) * textbuf->pitch +
            xstart + glyph->minx + gpos_xoffset;
            if ( dst < (Uint8*) textbuf->pixels ) {
                dst = (Uint8 *)textbuf->pixels;
            }
                ...
                const char *text, SDL_Color fg, SDL_Color bg)
{
    ...
    {
        const int idx = hb_glyph_info[i].codepoint;
        error = Find_Glyph(font, idx, CACHED_METRICS|CACHED_PIXMAP);
        if ( error ) {
            TTF_SetFTError("Couldn't find glyph", error);
            SDL_FreeSurface( textbuf );
            hb_buffer_destroy(hb_buffer);
                ...
                const char *text, SDL_Color fg)
{
    ...
    for (i = 0; i < glyph_count; i++)
    {
        ...
        for ( row = 0; row < glyph->pixmap.rows; ++row ) {
            ...
            (row+glyph->yoffset-gpos_yoffset) * textbuf->pitch/4 +
            xstart + glyph->minx + gpos_xoffset;
            if ( dst < (Uint32*) textbuf->pixels ) {
                dst = (Uint32 *)textbuf->pixels;
            }
...
/* Harfbuzz */
void TTF_SetDirection(int direction) /* hb_direction_t */
{
    g_hb_direction = (hb_direction_t)direction; // Jimmy
}

void TTF_SetScript(int script) /* hb_script_t */
{
    g_hb_script = (hb_script_t)script; // Jimmy
}
...
/* Tooh - Fix Harfbuzz in release mode */
void TTF_hb_buffer_set_direction(hb_buffer_t *buffer, hb_direction_t direction)
{
    hb_buffer_set_direction(buffer, direction);
}

void TTF_hb_buffer_set_script(hb_buffer_t *buffer, hb_script_t script)
{
    hb_buffer_set_script(buffer, script);
}

void TTF_hb_buffer_add_utf8(hb_buffer_t *buffer, const char *text, int text_length, unsigned int item_offset, int item_length)
{
    hb_buffer_add_utf8(buffer, text, text_length, item_offset, item_length);
}

void TTF_hb_shape(hb_font_t *font, hb_buffer_t *buffer, const hb_feature_t *features, unsigned int num_features)
{
    hb_shape(font, buffer, features, num_features);
}

void TTF_hb_font_destroy(hb_font_t *font)
{
    hb_font_destroy(font);
}

hb_glyph_info_t* TTF_hb_buffer_get_glyph_infos(hb_buffer_t *buffer, unsigned int *length)
{
    return hb_buffer_get_glyph_infos(buffer, length);
}

hb_glyph_position_t* TTF_hb_buffer_get_glyph_positions(hb_buffer_t *buffer, unsigned int *length)
{
    return hb_buffer_get_glyph_positions(buffer, length);
}

hb_buffer_t* TTF_hb_buffer_create(void)
{
    return hb_buffer_create();
}
    
void TTF_hb_buffer_destroy(hb_buffer_t *buffer)
{
